...
annualMixBoot <- function(x,aic.threshold=2,mode.thresh=10,init.mode,init.lam,init.var,ni=1) {
	...
	for(i in 1:ny) {
			...
			best.model[i] <- 1 #gaussian only
				if(ny==1) out[[i]] <- nMixNormalBoot(x,aic.threshold=aic.threshold,mode.thresh=mode.thresh,init.mode=init.mode, init.var=init.var, init.lam=init.lam,niter=ni)
				if(ny>1) out[[i]] <- nMixNormalBoot(x[,i],aic.threshold=aic.threshold,mode.thresh=mode.thresh,init.mode=init.mode[[i]], init.var=init.var[[i]], init.lam=init.lam[[i]],niter=ni)
			#run as normals only
